/**
 * Migración: Tickets (Incidentes) con un solo asignado → múltiples asignados.
 *
 * Copia `assignedTo` / `assignedToName` (o el legado V1 `fkxSoporte`) a las
 * listas `assignedToUids` / `assignedToNames`. Es idempotente: omite los
 * tickets que ya tienen la lista.
 *
 * Sin esta migración, los tickets antiguos no aparecen en las consultas y
 * notificaciones que ahora usan `array-contains` sobre `assignedToUids`.
 * `fkxSoporte` (V1) no se toca: sigue guardando solo al responsable principal.
 *
 ...
 *
 * Uso:
 *   node scripts/migrate-tickets-multi-asignado.js            # solo muestra lo que haría
 *   node scripts/migrate-tickets-multi-asignado.js --execute  # ejecuta la migración
 */
...
async function main() {
  ...
  const docs = await firestoreGet('Incidentes');
  console.log(`Tickets encontrados: ${docs.length}\n`);
  ...
  for (const doc of docs) {
    ...
    if (!uid) {
      // Sin asignado: nada que copiar.
      unassigned++;
      continue;
    }

    // El nombre V1 (`fkxSoporte`) tiene prioridad: es el dato que la app
    // mostraba en los tickets heredados.
    const name =
      f.fkxSoporte?.stringValue || f.assignedToName?.stringValue || '';
    const update = {
    ...
    if (!DRY_RUN) {
      await firestorePatch(`Incidentes/${id}`, update);
    }
    migrated++;
  }
  ...
  console.log(`  Sin asignado:    ${unassigned}`);
  if (DRY_RUN) {
