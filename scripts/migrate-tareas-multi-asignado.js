/**
 * Migración: Tareas con un solo responsable → múltiples responsables.
 *
 * Copia `assignedToUid` / `assignedToName` a las listas `assignedToUids` /
 * `assignedToNames`. Es idempotente: omite las tareas que ya tienen la lista.
 *
 * Sin esta migración, las tareas antiguas no aparecen en las consultas que
 * ahora usan `array-contains` sobre `assignedToUids` (mis tareas, badge de
 * pendientes, alertas de vencimiento).
 *
 ...
 *
 * Uso:
 *   node scripts/migrate-tareas-multi-asignado.js            # solo muestra lo que haría
 *   node scripts/migrate-tareas-multi-asignado.js --execute  # ejecuta la migración
 */
...
async function main() {
  ...
  const docs = await firestoreGet('Tareas');
  console.log(`Tareas encontradas: ${docs.length}\n`);
  ...
  for (const doc of docs) {
    ...
    // Ya migrada: tiene la lista con al menos un elemento.
    ...
    const uid = f.assignedToUid?.stringValue;
    if (!uid) {
    ...
    if (!DRY_RUN) {
      await firestorePatch(`Tareas/${id}`, update);
    }
    migrated++;
  }
